        ...
        <Show when={subtitleEnabled()}>
          ...
          >
            <For each={visibleCues()}>
              {(cue) => (
                <div data-start={cue.startSecond} onclick={() => moveToCueTime(cue)}>
                  ...
                  <label class="swap btn btn-ghost btn-sm btn-circle" onclick={(e) => copyToClipboard(cue, e)}>
                    <input type="checkbox" onclick={(e) => e.stopPropagation()} />
                    <IconCheck size={16} class="swap-on" />
                    <IconCopy size={16} class="swap-off" />
                  </label>
                </div>
              )}
            </For>
