...
from apps.common.util import AccessDate, GradingDate, OtpTokenDict, issue_active_context, track_fields
...
class Engagement(AttemptMixin):
    ...
    @classmethod
    async def start(cls, *, course_id: str, learner_id: str, lock: datetime):
        ...
        try:
            engagement = await Engagement.objects.acreate(
                course_id=course_id, learner_id=learner_id, active=True, lock=lock
            )
