...
from apps.learning.api.access_control import access_date, access_realm
...
async def start_engagement(request: HttpRequest, id: str):
    return await Engagement.start(
        course_id=id, learner_id=request.auth, lock=request.access_date["end"], realm=request.access_realm
    )
