...
import { tutorV1GetExamGrades, tutorV1GetModelInfo } from '@/api'
...
function RouteComponent() {
  ...
  const [info] = createCachedStore(
    'tutorV1GetModelInfo',
    () => ({ path: { app_label: 'exam', model: 'exam', id: params().id } }),
  ...
  return (
    <GradingProvider value={gradingStore}>
      <div class="space-y-8">
        <Breadcrumb app="exam" id={params().id} title={info.data?.title ?? ''} kind="grading" />
