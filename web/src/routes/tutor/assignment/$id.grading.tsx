...
import { tutorV1GetAssignmentGrades, tutorV1GetModelInfo } from '@/api'
...
function RouteComponent() {
  ...
  const [info] = createCachedStore(
    'tutorV1GetModelInfo',
    () => ({ path: { app_label: 'assignment', model: 'assignment', id: params().id } }),
  ...
  return (
    <GradingProvider value={gradingStore}>
      <div class="space-y-8">
        <Breadcrumb app="assignment" id={params().id} title={info.data?.title ?? ''} kind="grading" />
