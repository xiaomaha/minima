...
import { type ExamSpec, studioV1DeleteExamQuesion, studioV1SaveExamQuestion } from '@/api'
...
import { vExamQuestionEditingSpec } from './-data'
...
export const Question = (props: Props) => {
  ...
  const saveQuestion = async (validated: v.InferOutput<typeof vExamQuestionEditingSpec>) => {
    if (!staging.id) {
      showToast({
        title: t('Save failed'),
    ...
    const { data } = await studioV1SaveExamQuestion({
      path: { id: staging.id },
  ...
  const removeQuestion = async () => {
    ...
    await studioV1DeleteExamQuesion({ path: { id: staging.id, q: questionId } })
    batch(() => {
  ...
  return (
    <DataAction rootKey={['questionSet', props.index]} label={t('exam-question')} schema={vExamQuestionEditingSpec}>
      {(status, actions) => (
        <div class="relative">
          ...
          <Paper collapsed={!!source.questionSet[props.index]} fallback={<div class="line-clamp-1">{question().question}</div>}>
            <TextField
              ...
              schema={vExamQuestionEditingSpec.entries.question}
              multiline
            />
            ...
            <RichTextField
              ...
              label={t('Supplement')}
              schema={vExamQuestionEditingSpec.entries.supplement}
              setFiles={setFiles}
            />
            ...
            <NumberField
              ...
              label={t('Point')}
              schema={vExamQuestionEditingSpec.entries.point}
            />
            ...
            <Show when={question().format === 'single_choice' || question().format === 'number_input'}>
              <CommaSeparatedField
            ...
            <TextField
              path={['questionSet', props.index, 'solution', 'correctCriteria']}
              label={t('Correct criteria')}
              schema={vExamQuestionEditingSpec.entries.solution.entries.correctCriteria}
              multiline
            />
            <TextField
              path={['questionSet', props.index, 'solution', 'explanation']}
              label={t('Explanation')}
              schema={vExamQuestionEditingSpec.entries.solution.entries.explanation}
              multiline
            />
