...
import {
  ...
  studioV1GetAssignmentQuestions,
  studioV1SaveAssignmentQuestions,
} from '@/api'
...
import { EmptyQuestion, vAssignmentQuestionEditingSpec } from './data'
...
export const QuestionPool = () => {
  ...
  const saveAllQuestions = makeSaveQuestions(staging, source, fieldState, studioV1SaveAssignmentQuestions)
  const copyQuestionPool = makeCopyQuestionPool(staging, studioV1GetAssignmentQuestions)

  const completePercentage = createMemo((): number => {
    return questions().length > 0 ? 100 : 0
  })
  ...
  return (
    <>
      <DataAction
        rootKey={['questions']}
        label={t('Assignment questions')}
        schema={v.array(vAssignmentQuestionEditingSpec)}
      >
        {(_, actions) => (
          <>
