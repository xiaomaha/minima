...
import type { AssignmentSpec } from '@/api'
...
export const QuestionPool = () => {
  ...
  const { staging } = useEditing<AssignmentSpec>()
  ...
  const completePercentage = createMemo((): number => {
    return questionSet().length > 0 ? 100 : 0
  })
  ...
  return (
    <>
      <DataAction rootKey={['questionSet']} label={t('Assignment questions')} schema={v.array(vAssignmentQuestionEditingSpec)}>
        {(_, actions) => (
          <>
