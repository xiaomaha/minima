...
import { type ExamSpec, studioV1DeleteExamQuesion, studioV1SaveExamQuestions } from '@/api'
...
export const Question = (props: Props) => {
  ...
  const saveQuestion = async (validated: v.InferOutput<typeof vExamQuestionEditingSpec>) => {
    const { data } = await studioV1SaveExamQuestions({
      path: { id: staging.id },
  ...
  const removeQuestion = async () => {
    ...
    await studioV1DeleteExamQuesion({ path: { id: staging.id, question_id: questionId } })
    batch(() => {
      ...
      {(status, actions) => (
        <div class="relative">
          ...
          >
            ...
            <TextField
              path={['questions', props.index, 'solution', 'correctCriteria']}
