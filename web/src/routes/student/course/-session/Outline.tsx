...
import type { GradingCriterionSchema } from '@/api'
...
export const Outline = () => {
  ...
  const goToAssessment = (policy: GradingCriterionSchema) => {
    const id = policy.itemId
    if (policy.model === 'exam') {
      navigate({
        to: '/student/exam/$id/session',
        params: { id },
        search: (prev) => ({ ...prev, course: s().course.id }),
      })
    } else if (policy.model === 'assignment') {
      navigate({
        to: '/student/assignment/$id/session',
        params: { id },
        search: (prev) => ({ ...prev, course: s().course.id }),
      })
    } else if (policy.model === 'discussion') {
      navigate({
