...
import { discussionV1GetSession } from '@/api'
...
import { Discussion } from './-session/Discussion'
import { GettingStarted } from './-session/GettingStarted'
import { GradingReview } from './-session/GradingReview'

export const Route = createFileRoute('/student/discussion/$id/session')({
...
function RouteComponent() {
  ...
  const STEPS = [t('Getting started'), t('Discussion'), t('Grading Review'), t('Final score')]
        ...
        {(s) => (
          <SessionProvider value={store}>
            <div class="mx-auto max-w-5xl">
                ...
                <For each={STEPS}>
                  {(step, i) => (
                    <li
                      ...
                      class="step"
                      classList={{
                        'cursor-pointer step-info after:text-white': i() <= lastStep(),
                        'after:outline-4 after:outline-secondary': i() === activeStep(),
                      }}
                    >
                      <span
              ...
              <h2 class="text-center text-3xl font-bold my-12">{s().discussion.title}</h2>
                ...
                <Switch>
                  <Match when={s().step === 1}>
                    <div class="alert alert-warning">
                      ...
                      <div>
                        <div class="text-xs">
                          {t('The deadline for participating in the discussion is {{date}}.', {
              ...
              <Switch>
                ...
                <Match when={activeStep() === 3}>
                  <FinalScore session={s()} passingPoint={s().discussion.passingPoint} />
                </Match>
              </Switch>
            </div>
          </SessionProvider>
