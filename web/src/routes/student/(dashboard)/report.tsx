...
import { contentV1GetWatchedPublicMedias, learningV1GetReport, type WatchedMediaSchema } from '@/api'
...
export const Route = createFileRoute('/student/(dashboard)/report')({
...
function RouteComponent() {
  ...
  const [watched, setObserverEl, { refetch: refetchWatched }] = createCachedInfiniteStore(
    ...
    () => ({ query: range() }),
    async (options, page) =>
      (await contentV1GetWatchedPublicMedias({ ...options, query: { ...options.query, page } })).data,
  )

  const goToMedia = (watched: WatchedMediaSchema) => {
    navigate({
      to: '/student/media/$id',
      params: { id: watched.mediaId },
    ...
    <div class="max-w-5xl mx-auto space-y-16 flex flex-col">
      <div class="flex flex-col gap-8">
        <div class="flex justify-end gap-1">
          ...
          >
            {([key, label]) => (
              <input
                class="join-item btn btn-xs btn-ghost rounded-full border-0"
                type="radio"
                name="options"
                aria-label={label}
                checked={period() === key}
                onChange={() => setPeriod(key as keyof typeof period)}
              />
            )}
          </For>
        </div>
      ...
      <div class=" mx-auto space-y-2 flex flex-col w-full">
        ...
        <Show when={watched.end && watched.count === 0}>
          <NoContent message={t('No watched public media yet.')} />
        </Show>
